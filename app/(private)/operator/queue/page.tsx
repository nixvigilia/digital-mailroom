...
// Mock action requests - will be replaced with backend
...
const mockActionRequests: ActionRequest[] = [
  {
    id: "1",
    mailItemId: "mail-123",
    userId: "user-1",
  ...
  },
  {
    ...
    actionType: "scan",
    status: "in_progress",
    requestedAt: new Date("2025-01-14T14:20:00"),
    priority: "high",
    kycApproved: true,
    department: "Legal",
  },
...
export default function ActionQueuePage() {
  const [actionRequests, setActionRequests] = useState<ActionRequest[]>(mockActionRequests);
  ...
  const filteredRequests = actionRequests.filter((request) => {
    ...
    if (actionTypeFilter !== "all" && request.actionType !== actionTypeFilter) return false;
  ...
  const stats = {
    ...
    requiresApproval: actionRequests.filter((r) => r.status === "requires_approval").length,
  };
                ...
                {filteredRequests.map((request) => {
                  ...
                  return (
                    <Table.Tr key={request.id}>
                      ...
                      <Table.Td>
                        <Badge color={priorityColors[request.priority]} variant="light" size="sm">
                          {request.priority}
                        </Badge>
                      </Table.Td>
                      <Table.Td>
                      ...
                      <Table.Td>
                        <Group gap="xs">
                          <Tooltip label="View Details">
                            <Link href={`/operator/queue/${request.mailItemId}`}>
